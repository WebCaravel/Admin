...
>
    <div x-data="{ state: $wire.entangle('{{ $getStatePath() }}') }">
        @forelse($groups AS $optionGroupLabel => $optionGroup)
            <h2 class="font-bold mb-1">{{ $optionGroupLabel }}</h2>
            <div @class($class)>
                @foreach($optionGroup AS $option)
                    <label>
                        <input
                            ...
                            {{ $applyStateBindingModifiers('wire:model') }}="{{ $getStatePath() . "." . $option["id"] }}"
                            {{ $attributes->merge($getExtraAttributes())->class([
                                'transition duration-75 rounded shadow-sm focus:border-primary-500 focus:ring-2 focus:ring-primary-500 mr-2',
            ...
            <div @class($class)>
                @foreach($getOptions AS $option)
                    <label>
                        ...
                        {{ $applyStateBindingModifiers('wire:model') }}="{{ $getStatePath() . "." . $option["id"] }}"
                        {{ $attributes->merge($getExtraAttributes())->class([
                            'transition duration-75 rounded shadow-sm focus:border-primary-500 focus:ring-2 focus:ring-primary-500 absolute mr-2 mt-1',
                            'border-gray-300' => ! $errors->has($getStatePath()),
                            'border-danger-300 ring-danger-500' => $errors->has($getStatePath()),
                        ]) }}
                        />
